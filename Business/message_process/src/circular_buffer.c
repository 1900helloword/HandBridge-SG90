...
bool circular_buffer_write(circular_buffer *cb, const char *data, size_t length)
{
    ...
    DEBUG_PRINT("Attempt to acquire the write lock\n");
    mutex_lock(&cb->mutex); // 加锁，防止多线程同时访问缓冲区
    DEBUG_PRINT("Write lock has been acquired\n");
...
#else
        DEBUG_PRINT("Insufficient buffer space, unable to write.\n");
    ...
    DEBUG_PRINT("Write operation completed. Release write lock.\n");
    mutex_unlock(&cb->mutex); // 解锁
    return true;              // 数据写入成功
}
...
bool circular_buffer_read(circular_buffer *cb, char *data, size_t length)
{
    ...
    DEBUG_PRINT("Attempt to acquire a read lock\n");
    mutex_lock(&cb->mutex); // 加锁，防止多线程同时访问缓冲区
    DEBUG_PRINT("Read lock has been acquired\n");
    ...
    {
        DEBUG_PRINT("Insufficient buffer data, unable to read.\n");
        mutex_unlock(&cb->mutex); // 解锁
    ...
    DEBUG_PRINT("Read operation completed. Release read lock.\n");
    mutex_unlock(&cb->mutex); // 解锁
    return true;              // 数据读取成功
}
